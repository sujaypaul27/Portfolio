...
const Comment = memo(
  ({ comment, formatDate, index, isPinned = false, onDelete }) => {
    return (
      <div
      ...
      >
        ...
        <div className="flex items-start gap-3">
          {comment.profile_image ? (
            ...
            <div
              className={`p-2 rounded-full text-indigo-400 ${
...
const Komentar = () => {
  const handleDelete = async (id) => {
    try {
      ...
      // Your realtime subscription will auto refresh
    } catch (error) {
  ...
  const handleCommentSubmit = useCallback(
    async ({ newComment, userName, imageFile }) => {
      ...
      try {
        ...
        console.error("Error adding comment: ", error); // Log the actual error for debugging
      } finally {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-semibold text-white">
            Express your Comments / FeedBack{" "}
